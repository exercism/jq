...
@test error_message {
    run jq -Rrn --arg line '[ERROR]: Stack overflow' \
...
@test warning_message {
    run jq -Rrn --arg line '[WARNING]: Disk almost full' \
        'include "log-line-parser"; $line | message'
    assert_success
...
@test info_message {
    run jq -Rrn --arg line '[INFO]: File moved' \
        'include "log-line-parser"; $line | message'
    assert_success
...
@test message_with_leading_and_trailing_space {
    run jq -Rrn --arg line $'[WARNING]:   \tTimezone not set  \r\n' \
        'include "log-line-parser"; $line | message'
    assert_success
...
@test error_log_level {
    run jq -Rrn --arg line '[ERROR]: Disk full' \
...
@test warning_log_level {
    run jq -Rrn --arg line '[WARNING]: Unsafe password' \
        'include "log-line-parser"; $line | log_level'
    assert_success
...
@test info_log_level {
    run jq -Rrn --arg line '[INFO]: Timezone changed' \
        'include "log-line-parser"; $line | log_level'
    assert_success
    assert_output 'info' 
}
...
@test error_reformat {
    run jq -Rrn --arg line '[ERROR]: Segmentation fault' \
...
@test warning_reformat {
    run jq -Rrn --arg line '[WARNING]: Decreased performance' \
        'include "log-line-parser"; $line | reformat'
    assert_success
...
@test info_reformat {
    run jq -Rrn --arg line '[INFO]: Disk defragmented' \
        'include "log-line-parser"; $line | reformat'
    assert_success
...
@test reformat_with_leading_and_trailing_space {
    run jq -Rrn --arg line $'[ERROR]: \t Corrupt disk\t \t \r\n' \
        'include "log-line-parser"; $line | reformat'
    assert_success
